...
The per-panel hit-map sibling (``render_facets_figure_map``, FU-facet-
hitmap -- same image, plus one axes entry per panel + per-panel hit
elements, closing the "elements: [] + synthetic whole-image axes" gap
``routes.export_figures.export_figure_hitmap``'s facet branch used to
carry) lives in ``calc.figure_facets_map`` (split out to stay under the
500-line ceiling); it shares this module's ``draw_facet_grid`` so a faceted
preview's geometry always describes the SAME figure this module's own
bytes-only ``render_facets_figure`` produces -- one render path, not a
...
def draw_facet_grid(
    ...
    *,
    ...
    own per-series return -- consumed by ``figure_hitmap.collect_facet_map``
    to build a series line's pixel hit box without re-deriving it from
    ``ax.lines`` (a caller that doesn't need them, e.g.
    ``figure_page_facets.draw_facet_panel_cell``, simply ignores the return
    value; drawn-into-``axes`` side effects are unchanged).
...
def render_facets_figure(
    ...
    *,
    ...
    # Lazy import: the figure-building core is split into `figure_facets_map`
