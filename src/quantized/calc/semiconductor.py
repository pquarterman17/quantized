...
from quantized.calc._semiconductor_materials import MATERIALS, material_presets
from quantized.calc._semiconductor_materials import preset as _preset
...
# MATERIALS / material_presets / _preset moved to
# calc/_semiconductor_materials.py (2026-08-28, to stay under the 500-line
# module ceiling) -- imported above, re-exported here via __all__ so
# `semiconductor.material_presets()` / `semiconductor.MATERIALS` keep working
# for existing callers (routes/semiconductor.py, tests).
...
def carrier_concentration(nd: float, na: float, ni: float) -> dict[str, Any]:
    ...
    Do-not-"fix" to a naive ``n=0.5*(net+sqrt(...))``: for ``|net| >> ni``
    that catastrophically cancels. Solved via the larger root + mass-action
    law instead (see test_semiconductor.py); shared bug in MATLAB's
    ``carrierConcentration.m:45``.
    """
    if nd < 0 or na < 0:
