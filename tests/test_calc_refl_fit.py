...
# The fixtures carry isld +0.05e-6 / +0.08e-6 / +0.02e-6 as ABSORPTION (the fit
# API's convention; the generator converts for the engine, BUG-029) and are
# smeared with their own 1-sigma dQ column, so fits pass that column.
...
def xrr_channel() -> dict[str, Any]:
    ...
    return {"q": q, "r": r, "dr": dr, "dq": dq, "label": "xrr"}
...
def test_the_xrr_fixture_is_smeared_by_its_own_dq_column() -> None:
    # Pins the generator: fitting without the file's dQ column cannot reach
    # chi2 ~ 1 (measured 2026-09-24: 1.01 with it, 20.9 without).
    ch = xrr_channel()
    del ch["dq"]
    assert fit_reflectivity(bilayer(), [ch])["reduced_chi2"] > 5
...
def test_recovers_the_pnr_fixture_truth_including_magnetic_sld() -> None:
    ...
    out = fit_reflectivity(
        ...
        [
            {"q": q, "r": rpp, "dr": drpp, "dq": dq, "spin": "+", "label": "++"},
            {"q": q, "r": rmm, "dr": drmm, "dq": dq, "spin": "-", "label": "--"},
        ],
    )
    p = by_name(out)
    assert out["success"]
