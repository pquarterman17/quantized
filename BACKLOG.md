...
| **Native file/workspace handling — CORE SHIPPED** 2026-07-25 (`e5f04e1` + frontend): `qz --desktop` injects a native file-dialog bridge, picked paths import through the ordinary `/import` route and carry `source.path`, and user-picked files are consented so a NETWORK SHARE outside the allowed roots works at all. Browser mode degrades visibly via one shared entry point. **COMPLETE** — the working-path list (persisted; dialog opens there, picked folder floats up), Recent entries reopening their target with Locate/Remove/Retry, and the offline-vs-missing distinction all shipped the same day. Residual, not blocking: pywebview only (Tauri's dialog plugin is still Rust-only) and Recent covers files, not workspaces | MAIN #31 |
| **Durable autosave/recovery/trash — SLICE 1 SHIPPED** `6464498` (IndexedDB-backed store with 3 rotating generations, corrupt-newest fallback, legacy-slot migration, persistent health in the status bar). Slice 2 also shipped: crash detection via `lib/sessionMarker.ts`, so a restore after an unexpected close is announced while an ordinary one stays silent (restore stays unconditional by design — a prompt every launch would be worse for a workspace app). Slice 3 also shipped: dataset trash bounded by count (25) and age (7 days) with restore/purge (`store/trash.ts`). **COMPLETE** — the trash view shipped too (Data ▸ Trash; one-click restore, two-step permanent delete, eviction rules stated). Residual: datasets only, folders and other project objects are not covered | MAIN #32 |
...
| **Publication-quality clipboard copy — RESIDUAL ONLY** — SHIPPED 2026-07-25 (`70342fa` extracts the shared `buildFigureSpec`, `e45cb48` routes "Copy figure" through the publication renderer at 300 DPI; screen grab demoted to "Copy image (screen)"). Transparent-background preference SHIPPED `ac78218` (threaded through both render paths, route, spec and Preferences ▸ Plot; opaque default pinned by tests). **COMPLETE** — clipboard SVG shipped, probed via `ClipboardItem.supports` and omitted from the menu where unsupported rather than offered and failing. Residual: Windows EMF, which the sub-item scoped out | MAIN #35 |
