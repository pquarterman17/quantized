...
2. **New sub-plan `SILENT_STATE_CORRUPTION_PLAN.md` registered** (Parent
   ...
   backend D1–D8 (`#256`/`#257`) rows. Tasks #2/#4/#6–#9 stay open, a fix
   lane is in flight — booked in Actionable dev work below.
3. **The release-tracking docs were reconciled without ticking owner
   ...
   live candidate) got matching dated superseded notes — the next
   candidate is `v0.23.2-rc1`, cut after the Class B branch merges, no SHA
   yet. `RELEASE_BLOCKERS.md` checked: nothing rises to BLOCKER, its
...
| **Silent-state-corruption fix lane, tasks #2/#4/#6–#9** — class B (base-vs-computed-column strip mismatch) has no structural fix yet (#2); `store/derivedWorksheets.ts` drops a source's last column (#4); `applyCorrections`/`resetCorrections` delete the measurement column on the next apply or Reset — **data loss** (#6); a column-changing reimport leaves `fitSpec` stale (#7); `cat_levels` isn't remapped/stripped on formula-column removal (#8); `resetFigureDocumentForReshape` leaves `publication.seriesStyles` (#9). A fix lane is in flight | SILENT_STATE_CORRUPTION #2/#4/#6–#9 |
| DiraCulator follow-ups: history input-snapshot capture (card records don't embed the full input text) and the MATLAB Thin Film Card 6 Scherrer grain-size port (never ported, new PORT_CHECKLIST item) | DIRACULATOR_AUDIT (2026-08-22) |
...
| **DiraCulator: `substrates.critical_thickness` formula disagreement** — disagrees with MATLAB `+calc/+crystal/criticalThickness.m` four independent ways (~17.5× where both compute; `ValueError` on MATLAB's own docstring example); owner decision needed on the intended formula before the golden freeze can cover that op | DIRACULATOR_AUDIT (2026-08-15 finding) |
...
| `plans/ORIGIN_REPLACEMENT_ONE_WEEK_SPRINT.md` (+ `POST_SPRINT_INDEPENDENT_REVIEW.md`, `RELEASE_BLOCKERS.md`, `RC_RELEASE_NOTES_DRAFT.md`, `CHATGPT_SOL_TO_CLAUDE_RELEASE_HANDOFF.md`) | Active | RC published and engineering sprint complete (Day 7 all three states tracked separately). R1/R3/R4/R6/R7/R9 fixed + independently reviewed (#207/#208/#210/#211/#212/#213); R5 (Apple-silicon-only) decided; R8 (bundle headroom) closed twice over (C2, then #218). **Stable promotion happened 2026-08-28** (`v0.23.0`/`v0.23.1`) ahead of R2's full owner-acceptance checklist — see the dated note under `POST_SPRINT_INDEPENDENT_REVIEW.md`'s Stable-promotion gate and the Owner actions row below. `RELEASE_BLOCKERS.md`: nothing at BLOCKER, owner-verification list unchanged. Next acceptance candidate: `v0.23.2-rc1`, after the Class B branch below merges |
| `plans/SILENT_STATE_CORRUPTION_PLAN.md` | Active | Tasks #1 (Class A registration ratchet), #3 (`editableFigures.plot.view` remap), #5 (`reimportColumnsChanged` identity check) SHIPPED `#253`/`#254`; Completed-table F2/F5/F6 (`#255`) and backend D1–D8 (`#256`/`#257`) also shipped. Open: #2 (Class B — no distinct input type/assertion yet for stripping-vs-non-stripping `DataStruct` callers), #4 (`derivedWorksheets.ts` drops a source's last column), #6 (`applyCorrections`/`resetCorrections` delete the measurement column — data loss, refutes an earlier Completed-row claim), #7 (stale `fitSpec` after a column-changing reimport), #8 (`cat_levels` not remapped/stripped), #9 (`resetFigureDocumentForReshape` leaves `publication.seriesStyles`) |
| `plans/DIRACULATOR_AUDIT_PLAN.md` (Parent `PORT_PLAN.md` W4 / `PORT_CHECKLIST.md` W4) | Merged/frozen | Merged as `#143` (`ebe06c3`, 2026-08-16); 93-case MATLAB golden campaign frozen 2026-08-21 from `quantized_matlab@aee70d12` (248/248 tests, 0 skips). Open: history input-snapshot capture in card records (most card summaries don't embed the full input text — cosmetic, an "honest partial" on the acceptance gate), MATLAB Thin Film Card 6 Scherrer grain-size port (never ported, new PORT_CHECKLIST item), and an owner decision on the `critical_thickness` formula disagreement (see Owner actions) |
