...
| Item | Plan / item |
|------|-------------|
| Figure-authoring campaign, next slices — F2 broader parity (legacy + Graph Builder convergence on the canonical session, remaining Stage property bridge, direct-manipulation parity, render-path unification), F3 layout controls + export from PageDocument (F3.1–F3.4 shipped 2026-08-05/06; F3.5–F3.6 remain), then F4 recipes/templates + live grouping parity (F4.1, F4.2 rest, F4.3, F4.4); A1–A10 acceptance journeys are the exit gate | FIGURE_AUTHORING F2, F3.5–F3.6, F4 |
