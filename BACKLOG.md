...
| Figure-authoring campaign, next slices — F2 broader parity (legacy + Graph Builder convergence on the canonical session, F2.3's named panel set is EXHAUSTED as of 2026-08-12 — **shapes (F2.3c) 2026-08-11**, then **reference lines (F2.3d), tick formats (F2.3e), error-column bindings (F2.3f), channels reassignment (F2.3g), and group-by (F2.3h) all SHIPPED 2026-08-12**; what remains of F2.3 is F2.3i facet editing (BLOCKED — `bindings.facetKey` has no render wire; needs a multi-panel Publication Preview contract first) and region shades (owner call, see Owner actions); **F2.4 COMPLETE 2026-08-12** — F2.4c decor hit targets + F2.4d reference-line drag, then F2.4e shape drag closed the named gesture set; **F2.5 render-path unification COMPLETE 2026-08-11**, F2.5b closed the last reduced-path caller), F3 COMPLETE (F3.1–F3.6 shipped 2026-08-05/06/07), then F4 recipes/templates + live grouping parity (F4.1, F4.2 rest, F4.3, F4.4); A1–A10 acceptance journeys are the exit gate | FIGURE_AUTHORING F2, F4 |
...
| Item | Plan / item |
|------|-------------|
| **Library/workbook UX interview — remaining questions** — resume one at a time per the plan's pause point: tile-browser placement (L1.5 requires thorough mockups first — producing them is agent prep work), workbook child structure/ordering, view-switch continuity, workbook context-menu contents, Quick Figure Builder layout, ambiguity-confirmation UX, combine-files provenance, derived-data attachment, many-object scale. LQ.1 (template contract) CONFIRMED 2026-08-13 = L0.14; **no implementation authorized until the interview completes** | LIBRARY_WORKBOOK_UX interview (2026-08-13) |
...
| **Should Origin-decoded region shades be user-editable?** — `regionShades` (film-stack bands on an SLD profile) render on the Stage and, since 2026-08-11, in every export, but NO surface in the app creates, edits, or removes one: they exist only as `.opj`-decoded decor. F2.3d deliberately did NOT add editing to the Publication Preview alone, because that invents a capability the live plot lacks (the same call F2.3c made for shape `kind`). The consistent fix is a Stage card **and** a preview panel together — but the prior question is whether decoded provenance should stay immutable. Cheap either way; it needs a decision, not a design | FIGURE_AUTHORING F2.3 residue (2026-08-12) |
