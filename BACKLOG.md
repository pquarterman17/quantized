...
| **Durable autosave/recovery/trash — SLICE 1 SHIPPED** `6464498` (IndexedDB-backed store with 3 rotating generations, corrupt-newest fallback, legacy-slot migration, persistent health in the status bar). Remaining: startup recovery prompt gated on a clean-shutdown marker (today it still restores unconditionally), and the age/size-bounded project trash | MAIN #32 |
