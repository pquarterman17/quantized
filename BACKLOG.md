...
| Figure-authoring campaign, next slices — F2 broader parity (legacy + Graph Builder convergence on the canonical session, F2.3's named panel set is EXHAUSTED as of 2026-08-12 — **shapes (F2.3c) 2026-08-11**, then **reference lines (F2.3d), tick formats (F2.3e), error-column bindings (F2.3f), channels reassignment (F2.3g), and group-by (F2.3h) all SHIPPED 2026-08-12**; what remains of F2.3 is F2.3i facet editing (BLOCKED — `bindings.facetKey` has no render wire; needs a multi-panel Publication Preview contract first) and **F2.3j region-shade editing SHIPPED 2026-08-14** (Stage card + preview panel + the F2.1i `region_shades` override decomposed into the view, one slice; undo/redo proven behaviorally); **F2.4 COMPLETE 2026-08-12** — F2.4c decor hit targets + F2.4d reference-line drag, then F2.4e shape drag closed the named gesture set; **F2.5 render-path unification COMPLETE 2026-08-11**, F2.5b closed the last reduced-path caller), F3 COMPLETE (F3.1–F3.6 shipped 2026-08-05/06/07), then F4 recipes/templates + live grouping parity (F4.1, F4.2 rest, F4.3, F4.4); A1–A10 acceptance journeys are the exit gate (**A5's automated half SHIPPED 2026-08-14** — group-facet-journey.spec.ts, real-browser group build/edit/save/reopen/export; the checkbox itself stays owner-gated like A3/A4/A6) | FIGURE_AUTHORING F2, F4 |
