...
| Item | Plan / item |
|------|-------------|
| **Map ROI usability residue** (owner-reported 2026-08-12; the drag/Delete/✕ defects and the `x`/`y` labelling are all FIXED — see GUI_INTERACTION Completed). Remaining: (a) The box `▭` and ruler `▱` toolbar glyphs are near-identical at toolbar size, and both tooltips lead with shape rather than function. (b) A near-miss on a resize handle (>7 px) does not fail to resize — it starts a NEW box at the click point, destroying the old one, and `mapRoi` is deliberately out of undo so it is unrecoverable. (a) is mechanical; (b) is a real design call about whether one gesture should keep drawing-and-editing duty, with two sub-questions worth settling alongside it: should the box survive its tool being disarmed, and should `mapRoi` join the undo snapshot after all | GUI_INTERACTION (2026-08-12) |
