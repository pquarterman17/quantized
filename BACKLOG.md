...
| **Publication-quality clipboard copy — RESIDUAL ONLY** — SHIPPED 2026-07-25 (`70342fa` extracts the shared `buildFigureSpec`, `e45cb48` routes "Copy figure" through the publication renderer at 300 DPI; screen grab demoted to "Copy image (screen)"). Remaining: clipboard SVG where supported, and a transparent-background option — the latter is blocked on `calc/figure.py` never passing `transparent=` to `savefig`, so there is nothing to expose yet | MAIN #35 |
