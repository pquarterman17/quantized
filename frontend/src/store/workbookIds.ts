...
// nextDatasetId/nextFolderId/nextReportId in store/idSeq.ts: the two
// consumers — the import slice (PR A3) and the append path (PR A4) — land as
// parallel branches, and useApp.ts sits at its zero-headroom size ratchet; a
// shared prep module lets both fork without either touching that file. A
// separate counter is fine because uniqueness comes from the time prefix plus
// the monotonic suffix, not from sharing store/idSeq.ts's `_idSeq`.
//
