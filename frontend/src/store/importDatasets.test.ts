...
// BUG-006 site 10: the invariant this establishes is "a SAMPLED preview never
// carries a row-indexed sidecar". `book.preview.{time,values}` are decimated
// but `book.metadata` is the FULL book's, so a row-indexed sidecar would be
// paired with the wrong rows by every consumer that indexes it — measured on
// lib/barlayout.ts's label resolver, which returned ["A0","A0","B1"] for levels
// [0,1,2]: wrong category names that passed its own internal-consistency check.
describe("a sampled preview drops its row-indexed sidecars (BUG-006 site 10)", () => {
  const files = (...names: string[]) => names.map((n) => new File(["x"], n));
  const SIDECAR = { Group: ["A0", "A0", "B1", "B1", "C2", "C2"] };

  const payload = (previewSampled: boolean) => ({
    time: [0, 1],
    values: [[0], [1]],
    labels: ["Y"],
    units: [""],
    ...
    books: [
      { lazy: false as const, primary: true as const, id: "b0", labels: ["Y"], units: [""], metadata: { origin_book: "Primary" }, rows: 2, cols: 1 },
      {
        ...
        labels: ["Group"],
        units: [""],
        // The FULL book's sidecar (6 rows) beside a 4-row preview.
        metadata: { origin_book: "Lazy", text_columns: SIDECAR, origin_text_columns: SIDECAR, some_scalar: 7 },
        rows: 6,
        cols: 1,
        preview: { time: [1, 2, 4, 5], values: [[0], [1], [2], [2]] },
        preview_sampled: previewSampled,
      },
    ],
  });

  const importOne = async (previewSampled: boolean) => {
    vi.mocked(uploadFile).mockResolvedValueOnce(payload(previewSampled) as never);
    await useApp.getState().importFiles(files("p.opj"));
    return useApp.getState().datasets.find((d) => d.pending != null);
  };

  it("strips text_columns and origin_text_columns when the preview was SAMPLED", async () => {
    const ds = await importOne(true);
    expect(ds?.data.metadata?.["text_columns"]).toBeUndefined();
    expect(ds?.data.metadata?.["origin_text_columns"]).toBeUndefined();
  });

  it("keeps every NON-row-indexed metadata key — this is a targeted strip, not a wipe", async () => {
    const ds = await importOne(true);
    expect(ds?.data.metadata?.["some_scalar"]).toBe(7);
    expect(ds?.data.metadata?.["origin_book"]).toBe("Lazy");
  });

  it("KEEPS the sidecars for a merely padding-TRIMMED preview, whose cells do line up", async () => {
    // The half a row-count proxy could never get right: this preview is also
    // shorter than its sidecar, but it is a genuine PREFIX, so its labels are
    // correct and must survive.
    const ds = await importOne(false);
