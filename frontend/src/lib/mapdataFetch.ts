...
// grid — the backend `/api/plot/map` fetch + its pure client-side
// nearest-point regrid offline fallback (same pattern as plotdata.ts), plus
// the RSM axis-key helpers. Split out of `lib/mapdata.ts` (2026-08-23, C2
// bundle pass — see `frontend/scripts/check-bundle-size.mjs`'s header for the
// ratchet this split feeds): `lib/mapdata.ts` keeps only `is2DMap`/
// `canRenderMap`, the two capability checks `lib/quickPlot.ts`/
// `lib/plotSelectedTogether.ts`/`lib/stagetab.ts`/`components/Stage/Stage.tsx`
// need EAGERLY (import-time routing to the Map tab, and whether the tab
// exists at all) — every other export here (the actual map DATA fetch/regrid
// + the RSM angular/reciprocal-space axis toggle) is reached only from
// `MapStage`/`useMapRoi`/`useMapSectorWedge`/the ROI-cuts workshop, all
// already behind `MapStage`'s own `lazy()` boundary, never eager. Co-locating
// them in one file was dragging the whole fetch/regrid implementation (plus
// its own `lib/api` dependency) into the eager graph purely by file
// co-location — the same mechanism R8's `lib/api.ts`/primitives-barrel passes
// already fixed elsewhere. Verified before moving: neither `is2DMap` nor
// `canRenderMap` calls anything below internally, and every real (non-test)
// importer of what moved is lazy.
