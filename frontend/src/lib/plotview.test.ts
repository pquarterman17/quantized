...
describe("sanitizeView — regionShades (F2.3j region-shade editing)", () => {
  it("round-trips a valid shade unchanged", () => {
    const shade = { id: "sh1", x1: 0, y1: 1, x2: 2, y2: 3, fill: "#336699" };
    const out = sanitizePlotWindows(
      [win({ view: { ...defaultPlotView(), regionShades: [shade] } as PlotView })],
      new Set(["d1"]),
    );
    expect(out[0].view.regionShades).toEqual([shade]);
  });

  it("round-trips an explicit axis: 1 (secondary-Y shade)", () => {
    const shade = { id: "sh1", x1: 0, y1: 1, x2: 2, y2: 3, fill: "#336699", axis: 1 as const };
    const out = sanitizePlotWindows(
      [win({ view: { ...defaultPlotView(), regionShades: [shade] } as PlotView })],
      new Set(["d1"]),
    );
  ...
  it("never clamps a shade's coordinates — region shades have no page-anchor variant", () => {
    const shade = { id: "sh1", x1: -500, y1: 2000, x2: 1e6, y2: -1e6, fill: "#000000" };
    const out = sanitizePlotWindows(
      [win({ view: { ...defaultPlotView(), regionShades: [shade] } as PlotView })],
      new Set(["d1"]),
    );
  ...
  it("drops an entry with a missing id/fill, a non-finite coordinate, or a bad axis value", () => {
    const shades = [
      { id: "ok", x1: 0, y1: 0, x2: 1, y2: 1, fill: "#112233" },
      { x1: 0, y1: 0, x2: 1, y2: 1, fill: "#112233" }, // missing id
      { id: "s2", x1: 0, y1: 0, x2: 1, y2: 1 }, // missing fill
      { id: "s3", x1: Number.NaN, y1: 0, x2: 1, y2: 1, fill: "#112233" },
      "not-an-object",
    ];
    const out = sanitizePlotWindows(
      [win({ view: { ...defaultPlotView(), regionShades: shades } as unknown as PlotView })],
      new Set(["d1"]),
    );
    expect(out[0].view.regionShades).toEqual([{ id: "ok", x1: 0, y1: 0, x2: 1, y2: 1, fill: "#112233" }]);
  });

  it("drops an unrecognized axis value rather than trusting it verbatim", () => {
    const shade = { id: "sh1", x1: 0, y1: 0, x2: 1, y2: 1, fill: "#112233", axis: 7 };
    const out = sanitizePlotWindows(
      [win({ view: { ...defaultPlotView(), regionShades: [shade] } as unknown as PlotView })],
      new Set(["d1"]),
    );
    expect(out[0].view.regionShades).toEqual([{ id: "sh1", x1: 0, y1: 0, x2: 1, y2: 1, fill: "#112233" }]);
  });

  it("falls back to an empty array for a non-array regionShades field", () => {
    const out = sanitizePlotWindows(
      [win({ view: { ...defaultPlotView(), regionShades: "nope" } as unknown as PlotView })],
      new Set(["d1"]),
    );
    expect(out[0].view.regionShades).toEqual([]);
  });
});
...
// snapMovePosition / snapResizeSize (item 12 — edge/sibling snapping) moved
// to lib/windowSnap.test.ts alongside the extracted functions (F2.3j).
