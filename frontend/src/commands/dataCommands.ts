...
export function buildDataCommands(s: StoreGet): Action[] {
  return [
    ...
    { id: "stack-columns", group: "Data", section: "Combine & split", label: "Stack columns to long form…", description: "Reshape selected wide columns into value and category columns in long form.", run: () => runStackWorksheet(s) },
    { id: "unstack-columns", group: "Data", section: "Combine & split", label: "Unstack / pivot to wide form…", description: "Pivot category and value columns into separate columns in a wide worksheet.", run: () => runUnstackWorksheet(s) },
    { id: "join-by-key", group: "Data", section: "Combine & split", label: "Join datasets by key…", description: "Combine two datasets by matching values in selected key columns.", run: () => runJoinWorksheets(s) },
    {
      id: "tabulate",
      group: "Data",
    ...
    { id: "split", group: "Data", section: "Combine & split", label: "Split by column value…", description: "Create one derived dataset for each distinct value in a selected column.", run: () => { const id = s().activeId; if (id) s().openSplitDialog(id); } },
  ];
}
