...
import {
  SHOW_SQLITE_QUERY,
  listenForSqliteQuery,
  useSqliteQueryDialog,
} from "../../../store/sqliteQueryDialog";
import { useApp } from "../../../store/useApp";
import SqliteQueryDialog from "./SqliteQueryDialog";
...
describe("SqliteQueryDialog open/close bridge", () => {
  // The dialog is a lazyPanel gated on this flag (bundle pass 2026-08-29), so
  // the Data command's event has to reach the store while the dialog's chunk
  // is still unloaded -- that is the half AppOverlays wires up.
  it("opens the flag from the Data command event with no dialog mounted", () => {
    const stop = listenForSqliteQuery();
    expect(useSqliteQueryDialog.getState().open).toBe(false);
    window.dispatchEvent(new Event(SHOW_SQLITE_QUERY));
    expect(useSqliteQueryDialog.getState().open).toBe(true);
    stop();
  });

  it("stops hearing the event once unsubscribed", () => {
    const stop = listenForSqliteQuery();
    stop();
    window.dispatchEvent(new Event(SHOW_SQLITE_QUERY));
    expect(useSqliteQueryDialog.getState().open).toBe(false);
...
describe("SqliteQueryDialog", () => {
  it("loads the result as a dataset and closes", async () => {
    vi.mocked(querySqlite).mockResolvedValue({
      time: [1, 2], values: [[3], [4]], labels: ["signal"], units: [""], metadata: {},
    });
    useSqliteQueryDialog.setState({ open: true });
  ...
  it("Cancel closes without querying", () => {
    useSqliteQueryDialog.setState({ open: true });
    render(<SqliteQueryDialog />);
    fireEvent.click(screen.getByRole("button", { name: "Cancel" }));
    expect(useSqliteQueryDialog.getState().open).toBe(false);
    expect(querySqlite).not.toHaveBeenCalled();
  });
});
