...
export type AutosaveState = Pick<
  ...
  | "workbooks"
  | "savedRois"
  // Audit P2.8: the durable 2-D map view — persists (lib/workspace.ts) and
  // mutates on its own (store/mapView.ts's colour-limit/scale/colormap/slice/
  // annotation writers), so without a trigger here a colour limit typed into
  // the Inspector or a slice taken on the map left the title bar showing clean
  // right up to a crash. Same completeness-sweep class as `savedRois` above.
  | "mapView"
