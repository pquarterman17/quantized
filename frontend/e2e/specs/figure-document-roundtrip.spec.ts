...
test("an editable figure survives save, window close, and reopen byte-for-byte @core", async ({ page }) => {
  ...
  // A second window: the last remaining plot window is deliberately
  // close-protected (closeFigureWindow's ≥1-window invariant), so the journey
  // needs one to spare before it can close the figure's own window.
  await page.evaluate((view) => {
    const store = (window as unknown as { __qz: { useApp: { getState: () => Record<string, unknown> } } }).__qz.useApp;
    const state = store.getState() as unknown as {
      createWindow: (id: string | null) => string;
      activeId: string | null;
      setState?: unknown;
    };
    state.createWindow(state.activeId);
    (window as unknown as { __qz: { useApp: { setState: (patch: unknown) => void } } }).__qz.useApp.setState(view);
  }, RICH_VIEW);
  ...
  const savedDocument = JSON.parse(saved) as { id: string; name: string; plot: { view: Record<string, unknown> } };
