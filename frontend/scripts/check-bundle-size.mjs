 ...
 *  888,562 -> 876,452 after one real reduction, per rule 3's "deliberate diet
 *  pass" path (`measured + 1,024`, not the `total < budget - SLACK` forced
 *  path — this landed 13,134 B under the pin, inside the 40 kB `SLACK` band,
 *  so the forced check would not itself have fired; the pin is lowered here
 *  anyway so the gain is locked in rather than left as spendable slack).
 *
 ...
 *  reached only when `query.trim() === "" && rows.length === 0`, and PR C's
 *  own invariant (every dataset unconditionally yields a hierarchy worksheet
 *  node — `lib/libraryHierarchy.ts`) means that condition implies
 *  `datasets.length === 0`, so the branch never renders an actual row on ANY
 *  path today — deferring it costs nothing perceptible, first paint or
 *  restore, because nothing was ever painted there to begin with.
 *
 ...
 *    this commit                                       875,428   (−13,134 B)
 *  875,428 + 1,024 = 876,452, the same minimal-raise-margin convention
 ...
 *
 *  2026-08-23 (browser multi-tab lock, coordinator review round 3, N1-N6,
 *  `claude/browser-tab-lock`) — N2's event-driven re-engage
 *  (`installBrowserAutosaveReengage()`, three triggers: a definite-loss
 *  store watcher, `pageshow`, `visibilitychange`), N3's belt-and-braces
 *  Open-as-Copy refusal, N4's immediate post-recovery flush, and N6's
 *  once-per-transition health reporting all landed in the ALREADY-eager
 *  `useWorkspaceAutosave.ts`/`App.tsx`/`commands/projectLockCommands.ts` —
 *  none is a deferrable panel or user-action-gated module (the whole point
 *  is that they fire on browser lifecycle events with no user gesture),
 *  same irreducible session-lifecycle-store class the 2026-08-21/22/23
 *  entries above already established. `lib/browserLockProvider.ts` itself
 *  is UNCHANGED in reachability (still the same dynamic `import()` in
 *  App.tsx's non-desktop branch; verified: no `browserLockProvider` chunk
 *  in the build's entry script or modulepreload list). Measured 909,687 B
 *  (was 908,253 B the prior integrated measurement); budget re-set to
 *  measured + 1,024 = 910,711 per the same minimal-raise-margin convention
 *  — still 15.4 kB below the 926,154 B the R8 campaign started from. Full
 *  vitest (554 files / 8,434 tests), tsc --noEmit, and eslint all green
 *  post-raise.
 *
 *  2026-08-29 (#268 follow-up review: diagnostics storage allowlist + build
 *  identity, `claude/quantized-origin-jmp-gaps-vbtfnm`) — PIN UNCHANGED.
 *  Recorded because the measurement method matters more than the number
 *  here. Everything the review asked for (lib/storageKeys.ts's allowlist,
 *  the unrecognised-slot aggregate, the schema stamp) lives in the LAZY
 *  diagnostics chunk and measured +0 B eager, byte-identical to the base.
 *  The only eager cost was retiring plotRecipes.ts's `appVersion = "0"`
 *  placeholder, and the two ways of spending it were measured rather than
