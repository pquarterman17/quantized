...
**Closed for real 2026-08-23 (C2 continuation)** — the ~1 kB band above was
correctly flagged (by this same closure note, and by the release handoff) as
short of a *practical* working band, not the finish line. A follow-up C2 lane
re-profiled the integrated eager graph with real sourcemaps + a from-`main.tsx`
static-import reachability BFS (not chunk-name guessing) and found the flagged
`globalPlotRecipes` boundary from App.tsx's own comment ALREADY resolved by
#215/R8 (verified: `lib/plotRecipeStorage.ts`/`store/globalPlotRecipes.ts`/
`lib/plotRecipeMatch.ts`/the Recipe Manager panel are all confirmed small,
genuinely lazy chunks — no further gain there), then found and fixed five
more instances of the SAME `lib/api.ts`/primitives-barrel co-location
mechanism R8 itself had just fixed: `lib/pageDocument.ts`, `lib/figurepage.ts`,
`lib/mapdata.ts`, `lib/quickFigureMapping.ts`, `lib/datasetsplit.ts`, and
`lib/fitselection.ts` each had one small eager-reachable export (needed by
`parseWorkspace`, or by another eager function in the SAME file — two of
those were internal-call false positives a plain cross-file-importer grep
would have missed, ruled out explicitly before moving anything) dragging a
much larger sibling of edit/session/fetch-only logic into the eager graph.
Six pure module-boundary splits (full per-move deltas + rationale in
`scripts/check-bundle-size.mjs`'s own header), each measured before/after:
913,023 -> 904,976 B, a further 8,047 B (7.9 kB) recovery — 21,178 B
(20.7 kB) below the 926,154 B figure both R8 and #215 started from, clearing
the handoff's "at least 15 kB, preferably 20 kB+" target with margin.
`EAGER_JS_BUDGET` lowered 914,047 -> 906,000 (measured + 1,024). No behavior
change: `lib/workspace.ts`'s synchronous `parseWorkspace` and every recipe/
template behavior are unchanged (same exports, same signatures, same call
sites, just a different import path); no new Suspense fallback on any hot
path. Full vitest (551 files / 8,261 tests), `tsc --noEmit`, and `eslint
--max-warnings=0 src` all green; `npm run build` green, 1.0 kB under the new
budget. See the closure log's C2 row.
...
| 2026-08-23 | C2 — finish R8's re-opened bundle headroom target | branch `claude/c2-bundle-topup` | Claude Sonnet | Independent review pending (this row not yet reviewed) | Continuation of R8 per `CHATGPT_SOL_TO_CLAUDE_RELEASE_HANDOFF.md`'s C2 work item, starting from the 913,023 B integrated tree (914,047 budget). **Profiling method (never assumed from chunk names):** built with `vite build --sourcemap` and cross-referenced each large eager chunk's `sources`/`sourcesContent` against a from-`main.tsx` static-import reachability BFS (a throwaway local script, not committed) to tell genuinely-eager code apart from lazy-only code riding along by file co-location. First checked the handoff's named candidate — App.tsx's `globalPlotRecipes` dynamic-import comment, flagged as "worth re-measuring now that #215 made capture lazy" — and found it **already fully resolved**: `lib/plotRecipeStorage.ts`, `store/globalPlotRecipes.ts`, `lib/plotRecipe.ts`/`lib/plotRecipeMatch.ts`, `RecipeManagerPanel.tsx`, and `PlotRecipeApplyDialog.tsx` all confirmed (build output, not assumption) as separate, small, genuinely lazy chunks (1.5-4.6 kB each), none in the `<link rel=modulepreload>` eager set. No further gain available there. The real remaining lever was five more instances of the exact `lib/api.ts`/primitives-barrel co-location mechanism R8 had just fixed: a single small eager-reachable export (needed by `lib/workspace.ts`'s synchronous `parseWorkspace`, or — in two cases — called INTERNALLY by another eager function in the SAME file, which a plain cross-file-importer grep cannot see and produced two false-positive "candidates" ruled out before touching anything: `lib/foldertree.ts`'s `isSelfOrDescendant` is called by its own eager `moveFolder`; `lib/quickFigureMapping.ts`'s `roleFilteredYKeys`/`incompleteErrorNotices`/`axisDisplayName` are called by its own eager `canCreateQuickFigure`) was dragging a much larger sibling of edit-only/session-only/fetch-only logic into the eager graph. Six pure module-boundary splits, each verified zero-eager-consumer for what moved BEFORE moving it, each rebuilt and re-measured before the next (full per-move rationale + byte deltas in `scripts/check-bundle-size.mjs`'s own header, not duplicated here): `lib/pageDocument.ts` → new `lib/pageDocumentActions.ts` (913,023 → 912,010 B, -1,013 B); `lib/figurepage.ts` → new `lib/figurepageActions.ts` (912,010 → 910,071 B, -1,939 B); `lib/mapdata.ts` → new `lib/mapdataFetch.ts` (910,071 → 907,721 B, -2,350 B); `lib/quickFigureMapping.ts` → new `lib/quickFigureMappingActions.ts` (907,721 → 906,163 B, -1,558 B); `lib/datasetsplit.ts` → new `lib/datasetsplitDefault.ts` (906,163 → 905,994 B, -169 B — the smallest move, confirming even a tiny verified module-boundary split is never negative here, unlike a fresh dynamic-import boundary's fixed chunk-overhead tax); `lib/fitselection.ts` → new `lib/fitselectionActions.ts` (905,994 → 904,976 B, -1,018 B). **Net: 913,023 → 904,976 B, an 8,047 B (7.9 kB) further real, measured recovery — 21,178 B (20.7 kB) below the 926,154 B figure both R8 and #215 independently started from**, clearing the handoff's "at least 15 kB, preferably 20 kB+" target with margin (the 15 kB floor alone was cleared already at the third move). No behavior change anywhere: every move is a pure re-export-boundary split (same exports, same signatures, same call sites, just a different import path) — `lib/workspace.ts`'s synchronous `parseWorkspace` and every recipe/template behavior are byte-for-byte unchanged, and no new Suspense fallback becomes visible on any hot path (initial plot, Library, workspace restore all still resolve through the same already-eager entry points they always did). Roughly 30 import-site files updated mechanically (types kept with their still-eager owning module, moved functions repointed to the new sibling; several mixed-import call sites split into two import statements). Gates (frontend/, all foreground): `npx tsc --noEmit` clean; `npx eslint --max-warnings=0 src` clean; full `npx vitest run` — 551 files / 8,261 tests passed (foreground, the >480s auto-background waited out to completion, no self-polling); `npm run build` green — 883.8 kB eager vs the new 884.8 kB (906,000 B) budget, 1.0 kB under (measured + 1,024, same minimal-margin convention as every prior row). Did not touch `components/workshops/relink/`, `store/relink.ts`, `lib/relink.ts`, `lib/desktopBridge.ts`, `src/quantized/**`, or any calculator file, per this lane's explicit scope boundary. Committed on `claude/c2-bundle-topup`; not pushed, no PR opened — orchestrator reviews first per the lane's instructions. |
