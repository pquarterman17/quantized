...
| BUG-024 | P0 | Corrections / uncertainty propagation | Bound X/Y uncertainty columns are corrected as ordinary measurements, so offsets, backgrounds and nonlinear transforms silently manufacture wrong error magnitudes while the binding remains active | Codex | **FIXED 2026-09-20** — PR #385; error roles cross the route boundary, with hand-calculated coverage for additive, multiplicative, unit, normalization, footprint, and refusal semantics |
| BUG-025 | P0 | Desktop raw-source protection | A source recorded only on a workbook is absent from the declared-source guard, so Save As can overwrite that raw file after the workbook's final dataset is removed | Codex | **FIXED 2026-09-20** — PR #385; shared traversal protects and resolves dataset and workbook sources, with an end-to-end bridge refusal test |
...
- [ ] **Replace "refuse" with "resolve-then-apply."** The guard refuses; it does not
  ...
  **First implementation slice shipped 2026-09-20 — PR #389 / `e21cf32e`:** all five
  ...
  edit to an unrelated replacement dataset that reused the id.

  **Second implementation slice shipped 2026-09-20 — PR #390 / `04ed7499`:**
  worksheet row insertion/deletion, numeric and categorical cell edits, and block
  paste now resolve and resume automatically. Requests snapshot mutable row/edit
  arrays before awaiting, share one in-flight fetch, revalidate against the full
  row/level domain, preserve specific paste warnings, and create undo history only
  after the full-data edit actually runs.

  **Third implementation slice shipped 2026-09-20 — PR #391 / `8a072148`:**
  Worksheet Extract, Copy rows and Copy row now wait for full Origin data. The
  live and resumed paths share one pure filter/sort/exclusion resolver; bulk
  operations recompute value-based intent over the full book, while a single
  sampled row uses the validated preview-to-source map and refuses to guess when
  an old workspace has none.

  **Fourth implementation slice shipped 2026-09-20 — PR #392 / `bef49766`:**
  computed-column add/edit/remove now resume after resolution and revalidate on
  the full column layout. Persisted formulas are rebuilt over fetched measured
  data at the load boundary before a queued edit/removal can run; queued patches
  are immutable snapshots, formula-specific validation messages survive the
  resume wrapper, and invalid removal indices are true no-ops. Remaining refusal
  families: level order/recode and local analysis guards.

  **Fifth implementation slice shipped 2026-09-20 — PR #393 / `cdd71053`:**
  categorical level ordering and recode now resolve the full Origin book and
  replay the requested change against the resolved column identity. A newer
  panel edit made while loading cancels the queued request instead of applying
  stale intent; shifted computed categorical columns are re-resolved after
  measured data and formulas are restored; undo history is recorded exactly
  ...
  **Sixth implementation slice shipped 2026-09-20 — PR #394:**
  ...
  snapshots the selected columns/options, cancels if the user changes that
  setup while loading, and produces no partial result on load failure. Fit Y
  by X explicitly recomputes its report leg after hydration so a result
...
ordering note touches this entry: Escape still ranks `modal` surfaces by OPEN
order, while Tab and `inert` now follow the dialog painted on top. See R16 in
`plans/PRIMARY_SOFTWARE_AUDIT_PLAN.md`.
...
**Priority:** P3
**State:** Fixed 2026-09-20 — commit `26e5272e`
**Reported/investigated:** 2026-09-20 by ChatGPT-Sol
**Suggested owner/model:** Claude Haiku-class or inexpensive Codex backend model

Before the fix, `qz --version` exited as an unrecognized argument, forcing
release support to import Python internals to identify the installed build.

- [x] Add `--version` from the canonical `quantized.__version__` value.
- [x] Test both installed command aliases without launching the server/browser.
- [x] Add the probe to the wheel smoke test and run package gates.

Completion evidence and package-gate results are in
`POST_RELEASE_PROBLEM_AUDIT.md`; the implementation shipped in `26e5272e`.
...
**Status:** Fixed 2026-09-20 — PR #385 (`8760fbb6`)
...
**Priority:** P0
**Reported:** 2026-09-20 by Codex during a general bug hunt
**Status:** Fixed 2026-09-20 — PR #385 (`8760fbb6`); follow-up to BUG-002
...
**Priority:** P2
**Reported:** 2026-09-20 by Codex during a general bug hunt
**Status:** Fixed 2026-09-20 — PR #385 (`8760fbb6`); owner feel-check remains
...
**Priority:** P3
**Reported:** 2026-09-20 by Codex during a general bug hunt
**Status:** Fixed 2026-09-20 — PR #385 (`8760fbb6`); test infrastructure,
not a demonstrated product failure
