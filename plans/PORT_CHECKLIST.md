...
- [~] BG-from-file / fit-BG-from-region — BosonPlotter `onBGMouseUp` + `+utilities/estimateBackground.m`. **fit-BG-from-region backend done:** `calc/baseline.fit_region_background` (box-mask → raw-x `polyfit` → full-range background + region stats) + `POST /api/baseline/region`; golden `calc_bgregion.json` (linear/quadratic/cubic, coeffs ~1e-12; `bgrFreeze`). **Remaining:** frontend rubber-band box selection wiring + the separate BG-from-file (load a dataset as background) feature.
