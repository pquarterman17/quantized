...
- [~] Interactive render (uPlot 1D; Canvas2D for 2D maps) — uPlot 1D done (Stage: zoom/pan/cursor, overlays, ref lines, annotations, waterfall, per-series styling, grid/legend toggles). **2D backend foundation done** (`calc/map.py` `MapData`/`MapState`/`build_map`/`map_from_datastruct` over the parity-tested `regrid2d`; thin `POST /api/plot/map` regrids 3 scattered channels → grid payload; backend tests in `test_calc_map.py` + `test_api_plot.py`). **Canvas2D viewer (Stage) still TODO.**
