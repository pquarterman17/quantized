...
- [~] `importSPC` → `io/spc.py` — **independent implementation** against
  ...
  modern "new format LSB" (`fversn=0x4B`) **and, since 2026-07-25, the old
  pre-1996 format (`0x4D`)** are implemented — the corpus's first two real
  `0x4D` specimens unblocked and validated the old-format decoder, and the
  same corpus batch drove the TXYXYS fixes (directory-offset `fnpts`,
  per-subfile counts, no more silent single-subfile reads). All shipped in
  PR #89; PORT_PLAN #54 struck. MSB (`0x4C`) and Shimadzu (`0xCF`) remain
  recognized-but-rejected — still no example, do not guess. **Freeze
  pending: N/A (no MATLAB source exists).**
...
- [x] Neutron spin asymmetry — `+bosonPlotter/computeAsymmetryForExport.m` formula → `calc/reductions.py` — golden (positivity/NaN validity mask + exact error propagation; formula inlined in the freeze — the wrapper needs the GUI AppState). Pair *discovery* + consolidated-CSV wiring remain with the blocked polarized-CSV item (needs ++/−− metadata samples). Thin routes: `POST /api/reductions/{williamson-hall,fft-thickness,reflectivity-fft,spin-asymmetry}`. *Frontend dialogs shipped (MAIN_PLAN #11, 2026-07-11): `components/workshops/reductions/` — one ToolWindow, method picker (Williamson-Hall / FFT thickness / Reflectivity FFT), Analyze-menu entries. W-H peak entry is manual (the Peaks workshop's fit results aren't published to the store — no durable cross-workshop prefill source; documented follow-up, not built). FFT methods read the active dataset via `lib/rowstate.analysisData` and offer "→ Library" to save the FFT spectrum. Spin asymmetry frontend stays OUT — blocked on polarized (++/−−) metadata, same as the pair-discovery gap above.*
...
### XRD / RSM / surface
- [x] RSM analyze / strain — `+fitting/rsmAnalyze.m`, `+fitting/rsmStrain.m` — **both ported + golden.** `calc/rsm.rsm_strain` (strain via Q ratios, nominal lattices `a~2π/|Q|`, relaxation R; `POST /api/rsm/strain`). `calc/rsm_analyze.rsm_analyze` (separable Gaussian smooth → 3×3 local maxima + greedy min-sep → per-peak `surface_fit` in angle 2θ/ω **and** Q-space → centres/FWHM/substrate-film classification; `rsm_grids_from_datastruct` bridges the scattered RSM `DataStruct` → grids; `POST /api/rsm/analyze`). Full chain reachable: import RSM → `/api/rsm/analyze` → peaks → `/api/rsm/strain`. Golden `calc_rsm.json` (deterministic synthetic 2-Gaussian map: peak detection exact, per-peak fits centres ~1e-12/fwhm ~1e-9, strain chain + 3 closed-form cases @rtol 1e-6; `rsmFreeze`). Also `test_calc_rsm_analyze.py`, `test_api_rsm.py`. **MATLAB bug #6 found (2026-08-09, RSM_CUTS_PLAN item 23):** `+fitting/rsmStrain.m` guards `eps_parallel` only against the EXACTLY-degenerate case (`Qx == 0`), so a near-symmetric reflection (Qx nonzero but negligible relative to Qz) still runs `Qx_sub/Qx_film - 1` unguarded and returns a fabricated result — measured on the real corpus (`epytaxy_rsm.xrdml`'s substrate/film peak pair, both with `|Qx|/|Qz| ~ 1e-4`): `eps_parallel = 0.8087` (80.9%), not a physical strain, dominated entirely by fit noise in a Qx component barely distinguishable from zero. **Fixed in the port** (`calc/rsm.py::rsm_strain`, RSM_CUTS_PLAN #23): `eps_parallel` is now `NaN` plus a `warnings` entry whenever `|Qx|/|Qz| < tan(0.1°) ≈ 1.745e-3` for EITHER peak — a ratio threshold (not an FWHM-based one, to avoid rippling into routes/frontend) chosen ~10x above the measured fit-noise floor and ~5-7x below the repo's own golden asymmetric-reflection fixtures (0.88-1.3%), verified to leave `pytest -m golden -q` fully green (155 passed). **MATLAB side not fixed** (sibling-repo-first — deliberate separate work, never silent, and `Quantized_matlab` was not touched while investigating this); `Quantized_matlab/+fitting/rsmStrain.m` still carries the unguarded `Qx == 0`-only check.
...
- [x] Electrical / transport — `buildElectricalTab` — `calc/electrical.py` + `/api/electrical/*` + self-contained "Electrical" tab (resistivity/sheet-R/conductivity/mobility/current-density/Hall + Wiedemann-Franz/hall-analysis); reference-value tested. Commit `7fa8e0f`. **Golden verified** by the 93-case DiraCulator campaign frozen from `quantized_matlab@aee70d12` (`tests/test_calc_electrical_golden.py`); `van_der_pauw` remains labeled as a Python-only extension, not a parity claim.
- [x] Semiconductor — `buildSemiconductorTab` — `calc/semiconductor.py` (13 fns: intrinsic/carrier-conc/Fermi/built-in-V/depletion/Debye/Hall/Caughey-Thomas mobility…) + `/api/semiconductor/*` + tab. Commit `af3e661`. **Golden verified** by the 93-case DiraCulator campaign frozen from `quantized_matlab@aee70d12` (`tests/test_calc_semiconductor_golden.py`), including all 14 class-(a) `+calc` counterparts and byte-identical material presets.
- [x] Thin Film — `buildThinFilmTab` — `calc/thin_film.py` (deposition/sputter rate, Kiessig, Stoney stress, LSS range, multilayer-thermal, Scherrer grain size…) + `/api/thin-film/*` + tab. Commit `af3e661`; completed 2026-08-28. All MATLAB-backed operations are **golden verified** by the 94-case DiraCulator campaign frozen from `quantized_matlab@aee70d12` (`tests/test_calc_thin_film_golden.py`); `sauerbrey` is labeled as an extension.
- [x] Scherrer grain size (Thin Film Card 6) — `D = K·λ/(B·cosθ)`, K=0.9 (DiraCulator.m 2495-2510) — pure calc op + thin route + card + MATLAB GUI-formula freeze case completed 2026-08-28.
...
- [x] X-ray & Neutron (d-spacing, Q↔2θ, SLD) — `buildXrayNeutronTab` — d-spacing (Bragg) + Q↔2θ (`calc/xray.py`, `/api/xray/calc`, "X-ray" tab; commit `ad72c6c`); **SLD-from-formula** (`calc/sld_formula.py`, `/api/sld/formula`, "SLD" tab; commit `7e03a6d`) — neutron + X-ray SLD, real + imaginary (absorption), wavelength-dependent, via `periodictable` (the NIST NCNR engine); reference-value tested vs published NCNR values (Si/quartz/H₂O/D₂O/Fe/Gd). Six MATLAB-backed Bragg/Q modes are **golden verified** by the campaign frozen from `quantized_matlab@aee70d12` (`tests/test_calc_xray_golden.py`). d↔θ/d↔Q inverses, energy↔λ standalone modes, and `neutron_calc` are labeled extensions; SLD-from-formula remains a documented engine divergence verified against NIST values, not a MATLAB-parity claim.
- [x] Superconductor — `buildSuperconductorTab` — `calc/superconductor.py` (London/coherence depth, GL κ, critical fields, depairing current, BCS gap + presets) + `/api/superconductor/*` + tab. Commit `af3e661`. **Golden verified** by the campaign frozen from `quantized_matlab@aee70d12` (`tests/test_calc_superconductor_golden.py`), including both critical-field branches; `bcs_gap` remains labeled as an extension because MATLAB `bcsGap.m` is a different array-based fit routine.
- [x] Magnetic — `buildMagneticTab` — `calc/magnetic.py` (moment↔emu/Am²/µ_B, demag factors, Curie-Weiss µ_eff, Langevin, domain-wall); distinct from `calc/magnetometry.py`. Fixed MATLAB unit bugs (Curie-Weiss ~100×, domain-wall ×10) → froze GUI/textbook values. Commit `af3e661`. **Golden verified** by the campaign frozen from `quantized_matlab@aee70d12` (`tests/test_calc_magnetic_golden.py`); the Curie-Weiss and domain-wall divergences remain explicit and tested against the frozen MATLAB outputs with their documented corrections.
- [x] Optics — `buildOpticsTab` — `calc/optics.py` (Fresnel R/T, critical/Brewster angles, penetration & skin depth, n↔ε) + `/api/optics/*` + tab. Commit `13345d7`. **Golden verified** by the campaign frozen from `quantized_matlab@aee70d12` (`tests/test_calc_optics_golden.py`).
- [x] Vacuum — `buildVacuumTab` — `calc/vacuum.py` (mean-free-path, monolayer time, Knudsen, pump-down, sputter yield, gas-flow conductance) + `/api/vacuum/*` + tab. Fixed sputterYield GUI arg-order bug. Commit `13345d7`. **Golden verified** by the campaign frozen from `quantized_matlab@aee70d12` (`tests/test_calc_vacuum_golden.py`); the campaign confirms the arg-order bug is only in the MATLAB GUI call site, while `+calc/+vacuum/sputterYield.m` and the port agree.
- [x] Electrochemistry — `buildElectrochemistryTab` — `calc/electrochemistry.py` (Nernst, Butler-Volmer, Tafel, ohmic drop, double-layer C) + `/api/electrochemistry/*` + tab. Commit `13345d7`. **Golden verified** by the campaign frozen from `quantized_matlab@aee70d12` (`tests/test_calc_electrochemistry_golden.py`).
- [x] Thermal — `buildThermalTab` — `calc/thermal.py` (Wiedemann-Franz κ=L₀σT, Debye Θ_D, diffusivity α=κ/ρc_p) + `/api/thermal/*` + tab. Commit `13345d7`. **Golden verified** by the campaign frozen from `quantized_matlab@aee70d12` (`tests/test_calc_thermal_golden.py`); all three GUI-embedded formulas were frozen via cited inline replicas.
- [x] Diffusion — `buildDiffusionTab` — `calc/diffusion.py` (Arrhenius D, diffusion length √(Dt), Fick flux) + `/api/diffusion/*` + tab. Commit `13345d7`. **Golden verified** by the campaign frozen from `quantized_matlab@aee70d12` (`tests/test_calc_diffusion_golden.py`); `c_profile` remains labeled as an extension.
- [x] Substrates — `buildSubstratesTab` — `calc/substrates.py` (14-substrate reference DB + lattice mismatch + MATLAB-parity critical thickness) + `/api/substrates/*` + Elements-style reference tab. Commit `13345d7`; parity gap closed 2026-08-28. The table, lattice-mismatch behavior, and both frozen `criticalThickness` cases are **golden verified** against `quantized_matlab@aee70d12` (`tests/test_calc_substrates_golden.py`); the critical-thickness port preserves MATLAB's derived `b`, 60° factors, and 50-step iteration.
