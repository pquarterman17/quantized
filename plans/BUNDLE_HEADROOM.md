...
landed; it is 10,572 B now. The bundle pin is NOT edited in either direction,
...
|---|---|---:|
| parent (`git rev-parse HEAD~1`) | `c1757fb1` | **916,718** |
| this commit (its own SHA; the commit body repeats these numbers) | slice 3 | **909,828** |

Cumulative, in the order the seams were measured - each row is a full
`npm run build` on the same machine, same Node, `.vite` wiped:
...
first paint, autosave restore or crash recovery:

1. **Panel windows.** `WindowCanvas`'s `win.kind === "panel"` branch. A panel
   window exists only once the user composes one, and `PanelPlotWindow` was the
   only eagerly-reachable importer of `PanelCell` and `PanelOverlayWindow`, so
   all three left together. `lazy()` + `Suspense fallback={null}`, the shape
   `BackgroundPlotWindow` two lines above it already uses.
2. **Polar mode.** PlotStage's third runtime-conditional alternate render mode
   ...
   already `lazy()`). Entered exactly one way, the Plot toolbar's polar toggle.
   Takes `PolarStageCore` and `lib/polar.ts` with it.
...
`DRAGGED_OUT`. Measured with the guard's own `eagerlyReachable()` walk: 404
eager modules before, 398 after (three seams + four dragged out, minus the one
module added - `store/plotRecipeApplyLazy.ts`, the loader).
...
and nothing has mutated at that point. The two `lazy()` seams inherit the
...
#### Three seams measured and REJECTED, because they made the bundle BIGGER
...
pays import/export glue. Three seams were fully implemented, measured, and
reverted or kept on the measurement alone:

| rejected seam | attributed upper bound | measured |
|---|---:|---:|
| `store/reimport.ts` -> `lib/reimport.ts` + `lib/dependencyImpact.ts` | 1,830 B | **+1,451 B** |
| `commands/fileCommands.ts` -> `lib/originTemplate.ts` | 1,063 B | **+222 B** |
| `store/plotRecipes.ts` -> `store/plotRecipeApply.ts` | 2,857 B | -617 B (KEPT, but 78% of the bound was eaten) |
...
three `lib`/`store`-level ones realised -22%, -21% and +22%. A seam whose
target shares `plotview`/`formula`/`api`-class modules with the entry graph is
presumed a loss until a real build says otherwise.
