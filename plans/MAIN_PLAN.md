...
| `ORIGIN_REPLACEMENT_ONE_WEEK_SPRINT.md` (+ `POST_SPRINT_INDEPENDENT_REVIEW.md`, `RELEASE_BLOCKERS.md`, `RC_RELEASE_NOTES_DRAFT.md`, `CHATGPT_SOL_TO_CLAUDE_RELEASE_HANDOFF.md`) | The 2026-08-17 one-week push through the PRIMARY P1.1–P1.7 + Library H–N queue, its independent post-sprint audit/remediation, the release-blocker triage, and the RC/handoff tracking through v0.23.0's stable promotion | Time-boxed campaign with its own release-candidate cutting/qualifying mechanics and a distinct owner-acceptance gate (R2); layers on PRIMARY and LIBRARY_WORKBOOK_UX without duplicating their item registers |
| `SILENT_STATE_CORRUPTION_PLAN.md` | Two recurring silent-data-corruption defect classes (stale column-index-keyed state; base-vs-computed-column strip mismatch) found by a 2026-08-27/28 adversarial bug hunt — structural fixes plus the individual instances | Own defect-class register distinct from ordinary feature work; several instances are release-blocking data-loss bugs |
