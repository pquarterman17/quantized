...
**Created:** 2026-07-25
**Updated:** 2026-09-06 (later still): **P1.7 Pack Project PR 3, frontend
half** — the `kind: "bundle"` dataset-source extension (read at parse time
with a known `projectDir`, written back at serialize time only when saving
into that same directory) — see the new note under PR 3 below; the backend
publish/copy half (PR 2/3 proper) is still not shipped. Earlier the same
day: **P1.7 Pack Project PR 1** —
