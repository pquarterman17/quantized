...
- `plans/design/` — the Codex Design "Quantized Design System" handoff
...
- **Do not reintroduce byte-17 `connect=segment2`.** PRs #30/#31 were reverted
  by #32 after a corpus-wide false positive broke continuous curves. Origin
  line and line+symbol curves remain continuous unless a controlled specimen,
  COM/LabTalk oracle, rendered-Origin reference, and plausible corpus
  distribution independently prove another mode.
- **Protect multi-X worksheets.** Origin commonly stores `X,Y,X,Y,X,Y`; each Y
  binds to its nearest-preceding X-designated column. Frontend overlay blocks
  are keyed by `(dataset, x-channel)` and concatenated in draw/acquisition
  order. Never collapse them to the book's first X or form a sorted X union.
- **Protect non-monotonic X.** Hysteresis loops require `sorted: 0`, full-data
  X/Y extents, full-range line/marker path builders, and original row order.
  Any curve-render change must exercise a non-ascending fixture and verify
  interactive/export agreement.
- **Fail closed.** An explicitly referenced but undecoded X/Y is omitted and
  diagnosed; never coerce it to time/default X or another similarly named
  book/column.
...
- Codex should favor UI/rendering work and treat Claude's Origin backend
  contracts as deliberate. Do not reinterpret bytes, bindings, parser rules,
  or axis semantics without an explicit handoff and independent evidence.
